...
RTCManager::RTCManager(
    ...
    : config_(std::move(config)), receiver_(receiver), data_channel_(nullptr) {
    rtc::InitializeSSL();
    ...
    if (!config_.no_audio_device) {
        ...
        if (config_.disable_echo_cancellation) ao.echo_cancellation = false;
        if (config_.disable_auto_gain_control) ao.auto_gain_control = false;
        if (config_.disable_noise_suppression) ao.noise_suppression = false;
        if (config_.disable_highpass_filter) ao.highpass_filter = false;
        ...
        if (!audio_track_) {
            RTC_LOG(LS_WARNING)
                << __FUNCTION__ << ": Cannot create audio_track";
        }
    }
    ...
    if (video_track_source && !config_.no_video_device) {
        ...
        if (video_track_) {
            ...
            RTC_LOG(LS_WARNING)
                << __FUNCTION__ << ": Cannot create video_track";
        }
    }
}
...
std::shared_ptr<RTCConnection> RTCManager::CreateConnection(
    ...
    std::unique_ptr<PeerConnectionObserver> observer(new PeerConnectionObserver(
        sender, receiver_, &data_manager_dispatcher_));
    webrtc::PeerConnectionDependencies dependencies(observer.get());

    // WebRTC の SSL
    // 接続の検証は自前のルート証明書(rtc_base/ssl_roots.h)でやっていて、
    // その中に Let's Encrypt
    // の証明書が無いため、接続先によっては接続できないことがある。
    //
    ...
    if (!config_.proxy_url.empty()) {
        ...
        if (!URLParts::Parse(config_.proxy_url, parts)) {
            RTC_LOG(LS_ERROR)
                << "Failed to parse: proxy_url=" << config_.proxy_url;
            return nullptr;
        }
        ...
        if (!config_.proxy_password.empty()) {
            pi.password =
                rtc::CryptString(RawCryptString(config_.proxy_password));
        }
    ...
    if (data_manager_dispatcher_.Count() == 0 &&
        this->config_.create_data_channel) {
        std::string label("data_channel");
        std::cout << __FUNCTION__ << ": CreateDataChannel: " << label
                  << std::endl;
        ...
        data_manager_dispatcher_.OnDataChannel(data_channel_);
    }
