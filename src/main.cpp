...
int main(int argc, char* argv[]) {
    ...
    std::unique_ptr<rtc::FileRotatingLogSink> log_sink(
        new rtc::FileRotatingLogSink("./", "webrtc_logs",
...
#if USE_NVCODEC_ENCODER
    // NvCodec が有効な環境で HW MJPEG デコーダを使う場合、CUDA
    // が有効である必要がある
    ...
    auto capturer = ([&]() -> rtc::scoped_refptr<ScalableVideoTrackSource> {
        ...
        if (args.screen_capture) {
            ...
            if (!ScreenVideoCapturer::GetSourceList(&sources)) {
                RTC_LOG(LS_ERROR)
                    << __FUNCTION__ << "Failed select screen source";
                return nullptr;
            }
        ...
        if (v4l2_config.use_native) {
            ...
            mmal_config.native_frame_output =
    ...
    if (args.use_sdl) {
        sdl_renderer.reset(new SDLRenderer(
            args.window_width, args.window_height, args.fullscreen));
    ...
    if (rtc_manager == nullptr) {
        rtc_manager.reset(new RTCManager(std::move(rtcm_config),
                                         std::move(capturer), nullptr));
    }

    {
        ...
        if (!args.serial_device.empty()) {
            data_manager = std::shared_ptr<RTCDataManager>(
                SerialDataManager::Create(ioc, args.serial_device,
                                          args.serial_rate)
                    .release());
            if (!data_manager) {
                return 1;
...
#ifdef USE_NNG
        std::shared_ptr<NNGDataManager> nng_manager = nullptr;
        if (args.use_nng && (use_ayame || use_test)) {
            std::cout << "enable NNGDataManager" << std::endl;
            nng_manager =
                std::shared_ptr<NNGDataManager>(nng_sender->GetDataManager());
            rtc_manager->AddDataManager(nng_manager);
        }
#endif
        ...
        if (use_sora) {
            ...
            // SoraServer を起動しない場合と、SoraServer を起動して --auto
            // が指定されている場合は即座に接続する。 SoraServer を起動するけど
            // --auto が指定されていない場合、SoraServer の API
            // が呼ばれるまで接続しない。
            if (args.sora_port < 0 ||
            ...
            if (args.sora_port >= 0) {
                ...
                SoraServer::Create(ioc, endpoint, sora_client,
                                   rtc_manager.get(), config)
                    ->Run();
            }
