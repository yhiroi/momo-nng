...
SDLRenderer::SDLRenderer(int width, int height, bool fullscreen)
    ...
    if (SDL_Init(SDL_INIT_VIDEO) < 0) {
        RTC_LOG(LS_ERROR) << __FUNCTION__ << ": SDL_Init failed "
                          << SDL_GetError();
        return;
    }
    ...
    while (running_) {
        start_time = SDL_GetTicks();
        {
            ...
            for (const VideoTrackSinkVector::value_type& sinks : sinks_) {
                ...
                // flip (自画像とか？)
                // SDL_RenderCopyEx(renderer_, texture, &image_rect, &draw_rect,
...
SDLRenderer::Sink::~Sink() { track_->RemoveSink(this); }
...
webrtc::Mutex* SDLRenderer::Sink::GetMutex() { return &frame_params_lock_; }

bool SDLRenderer::Sink::GetOutlineChanged() { return !outline_changed_; }

int SDLRenderer::Sink::GetOffsetX() { return outline_offset_x_ + offset_x_; }

int SDLRenderer::Sink::GetOffsetY() { return outline_offset_y_ + offset_y_; }
...
int SDLRenderer::Sink::GetWidth() { return width_; }

int SDLRenderer::Sink::GetHeight() { return height_; }

uint8_t* SDLRenderer::Sink::GetImage() { return image_.get(); }
...
void SDLRenderer::SetOutlines() {
    ...
    for (int i = 0; i < sinks_count; i++) {
        ...
        RTC_LOG(LS_VERBOSE)
            << __FUNCTION__ << " offset_x:" << offset_x
            << " offset_y:" << offset_y << " outline_width:" << outline_width
