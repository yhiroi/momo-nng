...
static void add_optional_bool(CLI::App* app, const std::string& option_name,
...
void Util::ParseArgs(int argc, char* argv[], bool& use_test, bool& use_ayame,
                     bool& use_sora, int& log_level, MomoArgs& args) {
    ...
    app.add_flag("--use-nng", args.use_nng,
                 "Show video using ZeroMQ (if ZeroMQ is available and SDL is "
                 "not enabled)")
        ->check(is_nng_available);
    app.add_flag("--create-data-channel", args.create_data_channel,
                 "Create Data Channel");
    ...
    auto log_level_map =
        std::vector<std::pair<std::string, int>>({{"verbose", 0},
                                                  {"info", 1},
                                                  {"warning", 2},
                                                  {"error", 3},
                                                  {"none", 4}});
                 ...
                 "List available video encoders/decoders");
    {
        ...
        auto f = [](auto x) {
            return CLI::CheckedTransformer(
                VideoCodecInfo::GetValidMappingInfo(x), CLI::ignore_case);
        };
    ...
    test_app
        ->add_option("--port", args.test_port, "Port number (default: 8080)")
        ->check(CLI::Range(0, 65535));

    ayame_app
        ->add_option("--signaling-url", args.ayame_signaling_url,
                     "Signaling URL")
        ->required();
    ayame_app->add_option("--room-id", args.ayame_room_id, "Room ID")
        ->required();
    ...
    if (version) {
        ...
        std::cout << "USE_MMAL_ENCODER=" BOOST_PP_STRINGIZE(USE_MMAL_ENCODER)
                                                            << std::endl;
        std::cout << "USE_JETSON_ENCODER=" BOOST_PP_STRINGIZE(USE_JETSON_ENCODER) << std::endl;
        std::cout << "USE_NVCODEC_ENCODER=" BOOST_PP_STRINGIZE(USE_NVCODEC_ENCODER) << std::endl;
...
std::string Util::GenerateRandomChars() { return GenerateRandomChars(32); }
